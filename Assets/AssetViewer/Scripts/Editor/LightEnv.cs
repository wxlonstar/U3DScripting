    ...
    public class LightEnv : ScriptableObject {
        ...
        */
        ...
        public void UseLightFromScriptObject() {
            if(this.lightNames.Length <= 0) {
                Debug.Log("No direction lights in lightEnv");
                return;
            }
        ...
        [SerializeField]
        private Cubemap[] additionalReflectionCubeMaps;

        public void SaveAdditionalProbes() {
            ReflectionProbe[] probes = GameObject.FindObjectsOfType<ReflectionProbe>();
            this.additionalReflectionCubeMaps = new Cubemap[probes.Length];
            for(int i = 0; i < probes.Length; i++) {
                if(probes[i].mode == ReflectionProbeMode.Custom) {
                    if(probes[i].customBakedTexture != null) {
                        //Debug.Log(AssetDatabase.GetAssetPath(probes[i].customBakedTexture));
                        string CubemapLocation = AssetDatabase.GetAssetPath(probes[i].customBakedTexture);
                        additionalReflectionCubeMaps[i] = AssetDatabase.LoadAssetAtPath<Cubemap>(CubemapLocation);
                    }
                } else if(probes[i].mode == ReflectionProbeMode.Baked) {
                    if(probes[i].bakedTexture != null) {
                        //Debug.Log(AssetDatabase.GetAssetPath(probes[i].bakedTexture));
                        string CubemapLocation = AssetDatabase.GetAssetPath(probes[i].bakedTexture);
                        additionalReflectionCubeMaps[i] = AssetDatabase.LoadAssetAtPath<Cubemap>(CubemapLocation);
                    }
        ...
        public bool HasAdditionalProbes() {
            if(this.additionalReflectionCubeMaps.Length >= 1) {
                return true;
            } else {
                return false;
            }
        ...
        private int additionalProbeIndex = 0;
        public void UseAdditionalReflectionProbe() {
            //Debug.Log(additionalProbeIndex);
            RenderSettings.defaultReflectionMode = DefaultReflectionMode.Custom;
            RenderSettings.customReflection = this.additionalReflectionCubeMaps[additionalProbeIndex];
            additionalProbeIndex++;
            if(additionalProbeIndex == this.additionalReflectionCubeMaps.Length) {
                additionalProbeIndex = 0;
            }
        }
        ...
        public static void RestoreSavedLightEnv(EditorTool tool) {
            //Debug.Log(currentLights.Count + " dir-light restored.");
            if(EditorTools.activeToolType.Name != "LightEnvToolV2") {
                if(currentLights != null) {
                    if(currentLights.Count >= 1) {
                        foreach(GameObject light in currentLights) {
                            if(light != null) {
