...
[CustomEditor(typeof(MeshPainterV3))]
[CanEditMultipleObjects]
public class MeshPainterStyleV3 : Editor {
    ...
    void layerTex() {
        ...
        texLayer[0] = AssetPreview.GetAssetPreview(Select.gameObject.GetComponent<MeshRenderer>().sharedMaterial.GetTexture("_TexLayer01")) as Texture;
        texLayer[1] = AssetPreview.GetAssetPreview(Select.gameObject.GetComponent<MeshRenderer>().sharedMaterial.GetTexture("_TexLayer02")) as Texture;
        texLayer[2] = AssetPreview.GetAssetPreview(Select.gameObject.GetComponent<MeshRenderer>().sharedMaterial.GetTexture("_TexLayer03")) as Texture;
        texLayer[3] = AssetPreview.GetAssetPreview(Select.gameObject.GetComponent<MeshRenderer>().sharedMaterial.GetTexture("_TexLayer04")) as Texture;
    }
    ...
    bool Check() {
        bool Check = false;
        Transform Select = Selection.activeTransform;
        Texture ControlTex = Select.gameObject.GetComponent<MeshRenderer>().sharedMaterial.GetTexture("_ControlMap");
        if(Select.gameObject.GetComponent<MeshRenderer>().sharedMaterial.shader == Shader.Find("SoFunny/shd_layeredTerrain_v3") || Select.gameObject.GetComponent<MeshRenderer>().sharedMaterial.shader == Shader.Find("SoFunny/shd_layeredTerrain_v3")) {
            if(ControlTex == null)
            {
    ...
    void setContolTex(string path) {
        ...
        Selection.activeTransform.gameObject.GetComponent<MeshRenderer>().sharedMaterial.SetTexture("_ControlMap", ControlTex);
    }
    ...
    void Painter() {
        ...
        MaskTex = (Texture2D)CurrentSelect.gameObject.GetComponent<MeshRenderer>().sharedMaterial.GetTexture("_ControlMap");//get splat map in material
