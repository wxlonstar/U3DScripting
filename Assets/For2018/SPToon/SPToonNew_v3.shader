Shader "SoFunny/SPToon/SPToonNew_v3"
{
    Properties
    {
        [Header(MainToon)]
...
// #endif
                ...
                half4 frag(v2f i) : SV_Target
                {
                    ...
                    #if LIGHTMAP_ON
                    //光照贴图较暗处阴影淡化
                        half3 lightMapCol=saturate(SAMPLE_GI(i.lightmapUV, i.vertexSH, normalize(i.worldNormal.xyz)));
                        half lightmapVal=saturate(lightMapCol.r*0.333+lightMapCol.g*0.333+lightMapCol.b*0.333);
                        lightmapVal=smoothstep(0.6,1,lightmapVal)*_ShadowStrength;

                        shadowAtten=shadowAtten*lightmapVal+(1-lightmapVal);
